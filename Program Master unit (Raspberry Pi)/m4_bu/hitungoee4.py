...
def resetTrue(reset):
    ...
    global olAnginhours, olAnginhours, olAnginminutes, olAnginseconds, otherlossAngin, otherloss
    global olTotalhours, olTotalhours, olTotalminutes, olTotalseconds, otherlossTotal, otherloss
...
# *inisialisasi untuk getdataloop
    ...
    def getDataLoop(self, URLloop):
        ...
        self.process = int(self.data2['process']['process4'])
...
# void setup
URLweb = "http://polmanpms.online/client"
URLloop = "http://polmanpms.online/client/loop"
URLOee = "http://polmanpms.online/client/postoee"
...
while True:
    ...
    if status != "sd":
        ...
        print("status blink di while true adalah {}".format(status))
    ...
    # -----------------------------------------------------
        ...
        # otherloss = otherlossMP + otherlossAngin
    ...
    #     # *speed_loss = pLoss (performance loss), breakdown_loss = 0 (tidak digunakan), downtimeLoss = dtLoss (downtime loss pa wahyu)
        ...
        latencyAwal = datetime.datetime.now()

        json.OEEposting(URLOee, availability, performance,
                        quality, nilaiOEE, pLoss, dummy, dtLoss, defecttimeLoss)
        latencyAkhir = datetime.datetime.now()
        hasilLatency = latencyAkhir - latencyAwal

        det = round(hasilLatency.total_seconds(), 2)

        print("hasil latency adalah {}".format(det))
        print("date time sekarang adalah {}".format(latencyAkhir))
